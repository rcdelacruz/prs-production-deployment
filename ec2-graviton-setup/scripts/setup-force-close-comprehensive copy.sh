...
#
# Prerequisites:
#   - Docker containers must be running (prs-ec2-postgres)
...
if [[ ! -f "$PROJECT_DIR/docker-compose.yml" ]]; then
    echo -e "${RED}Error: This script must be run from the ec2-graviton-setup directory${NC}"
    echo -e "${RED}Current directory: $(pwd)${NC}"
    echo -e "${RED}Expected directory: .../prs-production-deployment/ec2-graviton-setup${NC}"
    exit 1
fi

# Check if Docker containers are running
echo -e "${YELLOW}Checking if Docker containers are running...${NC}"
if ! docker ps | grep -q "prs-ec2-postgres"; then
    echo -e "${RED}Error: prs-ec2-postgres container is not running${NC}"
    echo -e "${YELLOW}Please start the containers first:${NC}"
    echo -e "${YELLOW}  ./scripts/deploy-ec2.sh start${NC}"
    exit 1
fi
...
# Set default values if not found in .env (production values)
...
if ! docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -c 'SELECT 1;'" > /dev/null 2>&1; then
...
validate_force_close_results() {
    ...
    RS_STATUS=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT status FROM requisitions WHERE rs_number = '$rs_number';\"" | tr -d ' ')
    ...
    RS_NOTES=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COALESCE(force_close_reason, 'NULL') FROM requisitions WHERE rs_number = '$rs_number';\"" | tr -d ' ')
    ...
    PENDING_CS=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM canvass_requisitions WHERE requisition_id = (SELECT id FROM requisitions WHERE rs_number = '$rs_number') AND status IN ('draft', 'for_approval');\"" | tr -d ' ')
    ...
    PENDING_IR=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM invoice_reports WHERE requisition_id = (SELECT id FROM requisitions WHERE rs_number = '$rs_number') AND status IN ('draft', 'pending');\"" | tr -d ' ')
    ...
    PO_NOTES=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COALESCE(system_generated_notes, 'NULL') FROM purchase_orders WHERE requisition_id = (SELECT id FROM requisitions WHERE rs_number = '$rs_number') LIMIT 1;\"")
    ...
    OFM_ITEMS=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM requisition_item_lists WHERE requisition_id = (SELECT id FROM requisitions WHERE rs_number = '$rs_number') AND item_type = 'ofm';\"" | tr -d ' ')
...
ITEM_COUNT=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM items WHERE id IN (7, 28);\"" | tr -d ' ')
...
SUPPLIER_COUNT=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM suppliers;\"" | tr -d ' ')
...
docker exec -i prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME" << 'EOF'
-- ============================================================================
...
-- Add items to Scenario 5 requisition (OFM only - RS Type determines item type)
INSERT INTO requisition_item_lists (requisition_id, item_id, item_type, quantity, notes, account_code, created_at, updated_at)
VALUES
...
((SELECT id FROM requisitions WHERE rs_number = 'TEST-FC-SCENARIO5' LIMIT 1), 8, 'ofm', 50, 'Item 2 - RS not yet in progress (OFM)', '12345', NOW(), NOW());
...
-- Add items to Scenario 6 requisition (OFM only - RS Type determines item type)
INSERT INTO requisition_item_lists (requisition_id, item_id, item_type, quantity, notes, account_code, created_at, updated_at)
VALUES
...
((SELECT id FROM requisitions WHERE rs_number = 'TEST-FC-SCENARIO6' LIMIT 1), 8, 'ofm', 50, 'Item 2 - PO not yet for delivery (OFM)', '12345', NOW(), NOW());
...
-- Add items to Scenario 7 requisition (OFM only - RS Type determines item type)
INSERT INTO requisition_item_lists (requisition_id, item_id, item_type, quantity, notes, account_code, created_at, updated_at)
VALUES
...
((SELECT id FROM requisitions WHERE rs_number = 'TEST-FC-SCENARIO7' LIMIT 1), 8, 'ofm', 50, 'Item 2 - no deliveries yet (OFM)', '12345', NOW(), NOW());
...
-- ============================================================================
-- This creates: RS → Canvass → PO (FOR_DELIVERY) → Multiple Partial Deliveries → Incomplete Payment
-- Example: RS=10pcs, PO=5pcs@100=500, DR1=3pcs=300, DR2=1pc=100, PR=300 (only DR1 paid)
-- Validation: Total PR amount (300) ≠ Total DR amount (400) = UNPAID DELIVERIES
-- Result: Force close button should be DISABLED (User should pay all delivered quantities first)
...
-- Add items to Scenario 8 requisition (OFM only - following your example: Item A = 10 pcs)
INSERT INTO requisition_item_lists (requisition_id, item_id, item_type, quantity, notes, account_code, created_at, updated_at)
VALUES
((SELECT id FROM requisitions WHERE rs_number = 'TEST-FC-SCENARIO8' LIMIT 1), 7, 'ofm', 10, 'Item A - unpaid deliveries example (OFM)', '12345', NOW(), NOW());
...
-- Add canvass items for Scenario 8 (CS#1 = Item A - 5 pcs with unit price 100)
INSERT INTO canvass_items (
    canvass_requisition_id, requisition_item_list_id, status,
    created_at, updated_at
) VALUES
...
-- Create canvass item suppliers for Scenario 8 (5 pcs @ 100 pesos = 500 total)
INSERT INTO canvass_item_suppliers (canvass_item_id, supplier_id, term, quantity, "order", unit_price, discount_type, is_selected, supplier_type, created_at, updated_at)
VALUES
((SELECT id FROM canvass_items WHERE canvass_requisition_id = (SELECT id FROM canvass_requisitions WHERE cs_number = 'FC-CS-08' LIMIT 1) AND requisition_item_list_id = (SELECT id FROM requisition_item_lists WHERE requisition_id = (SELECT id FROM requisitions WHERE rs_number = 'TEST-FC-SCENARIO8' LIMIT 1) AND item_id = 7)),
 (SELECT id FROM suppliers LIMIT 1), '30 days', 5, 1, 100.00, 'fixed', true, 'supplier', NOW(), NOW());

-- Add PO items for Scenario 8 (PO#1 = Item A - 5 pcs = 500 pesos)
INSERT INTO purchase_order_items (
    purchase_order_id, canvass_item_id, requisition_item_list_id, quantity_purchased, canvass_item_supplier_id,
    created_at, updated_at
) VALUES
...
-- Create delivery receipts for Scenario 8 (DR#1 = 3 pcs = 300 pesos, DR#2 = 1 pc = 100 pesos)
INSERT INTO delivery_receipts (
    dr_number, requisition_id, po_id, supplier, is_draft, company_code, status,
    created_at, updated_at
) VALUES
...
-- Add delivery receipt items for Scenario 8 (DR#1 = 3 pcs delivered = 300 pesos)
INSERT INTO delivery_receipt_items (
    dr_id, po_id, item_id, po_item_id, item_des, qty_ordered, qty_delivered, unit,
    created_at, updated_at
) VALUES
((SELECT id FROM delivery_receipts WHERE dr_number = 'TEST-DR-008-1' LIMIT 1),
 (SELECT id FROM purchase_orders WHERE po_number = 'TEST-PO-008' LIMIT 1),
 7,
 ...
 (SELECT itm_des FROM items WHERE id = 7 LIMIT 1), 5, 3, (SELECT unit FROM items WHERE id = 7 LIMIT 1), NOW(), NOW());

-- Add delivery receipt items for Scenario 8 (DR#2 = 1 pc delivered = 100 pesos)
INSERT INTO delivery_receipt_items (
    dr_id, po_id, item_id, po_item_id, item_des, qty_ordered, qty_delivered, unit,
    created_at, updated_at
) VALUES
((SELECT id FROM delivery_receipts WHERE dr_number = 'TEST-DR-008-2' LIMIT 1),
 (SELECT id FROM purchase_orders WHERE po_number = 'TEST-PO-008' LIMIT 1),
 7,
 (SELECT id FROM purchase_order_items WHERE purchase_order_id = (SELECT id FROM purchase_orders WHERE po_number = 'TEST-PO-008' LIMIT 1) AND requisition_item_list_id = (SELECT id FROM requisition_item_lists WHERE requisition_id = (SELECT id FROM requisitions WHERE rs_number = 'TEST-FC-SCENARIO8' LIMIT 1) AND item_id = 7)),
 (SELECT itm_des FROM items WHERE id = 7 LIMIT 1), 5, 1, (SELECT unit FROM items WHERE id = 7 LIMIT 1), NOW(), NOW());

-- Update delivery receipts to calculate latest_delivery_status for Scenario 8
UPDATE delivery_receipts SET latest_delivery_status = 'Partially Delivered' WHERE dr_number = 'TEST-DR-008-1';
UPDATE delivery_receipts SET latest_delivery_status = 'Partially Delivered' WHERE dr_number = 'TEST-DR-008-2';

-- Create delivery receipt invoices for Scenario 8 (IR#1 for DR#1 only = 300 pesos, no IR for DR#2)
INSERT INTO delivery_receipt_invoices (
    delivery_receipt_id, invoice_no, issued_invoice_date, total_sales, vat_amount,
    created_at, updated_at
) VALUES
((SELECT id FROM delivery_receipts WHERE dr_number = 'TEST-DR-008-1' LIMIT 1),
 'INV-TEST-FC-008-1', NOW(), 300.00, 36.00,
 NOW(), NOW());
-- Note: No invoice created for DR#2 (TEST-DR-008-2) - this creates the payment mismatch
...
('FC-PR-08', 'A',
 (SELECT id FROM requisitions WHERE rs_number = 'TEST-FC-SCENARIO8' LIMIT 1),
 (SELECT id FROM purchase_orders WHERE po_number = 'TEST-PO-008' LIMIT 1),
 (SELECT id FROM delivery_receipt_invoices WHERE delivery_receipt_id = (SELECT id FROM delivery_receipts WHERE dr_number = 'TEST-DR-008-1' LIMIT 1)),
 'For PR Approval', false,
 NOW(), NOW());

-- Create invoice report for Scenario 8 (IR#1 = 300 pesos for DR#1 only, payment not yet approved)
INSERT INTO invoice_reports (
    ir_number, requisition_id, purchase_order_id, payment_request_id, status, is_draft,
    company_code, created_by, supplier_invoice_no, issued_invoice_date, invoice_amount,
    created_at, updated_at
...
('FC-IR-08',
 ...
 '12553', 150, 'INV-TEST-FC-008-1', NOW(), 300.00,
 NOW(), NOW());
...
if [ $? -eq 0 ]; then
    ...
    RS_COUNT=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM requisitions WHERE rs_number LIKE 'TEST-FC-%';\"" | tr -d ' ')
    echo -e "${GREEN}✓ Created $RS_COUNT test requisitions${NC}"

    # Check purchase orders were created
    PO_COUNT=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM purchase_orders WHERE po_number LIKE 'TEST-PO-%';\"" | tr -d ' ')
    echo -e "${GREEN}✓ Created $PO_COUNT test purchase orders${NC}"

    # Check payment requests were created
    PR_COUNT=$(docker exec prs-ec2-postgres bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM rs_payment_requests WHERE pr_number LIKE 'FC-PR-%';\"" | tr -d ' ')
