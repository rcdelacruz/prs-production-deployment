...
API_BASE_URL="https://localhost:8444/api/v1"
...
authenticate() {
    ...
    local auth_response=$(curl -s -k -X POST \
...
get_test_requisition_id() {
    ...
    local requisition_id=$(docker exec prs-local-postgres-timescale bash -c "
        PGPASSWORD='${POSTGRES_PASSWORD:-localdev123}' psql -U ${POSTGRES_USER:-prs_user} -d ${POSTGRES_DB:-prs_local} -t -c \"
...
test_post_force_close_validation() {
    ...
    local canvass_response=$(curl -s -k -X POST \
        -H "Authorization: Bearer $AUTH_TOKEN" \
        -H "Content-Type: application/json" \
        -d "$canvass_data" \
        "$API_BASE_URL/canvass/")

    local canvass_http_code=$(curl -s -k -X POST \
        -H "Authorization: Bearer $AUTH_TOKEN" \
        -H "Content-Type: application/json" \
        -d "$canvass_data" \
        -w '%{http_code}' \
...
test_detailed_requirements_validation() {
    ...
    local item_7_gfq=$(docker exec prs-local-postgres-timescale bash -c "PGPASSWORD='${POSTGRES_PASSWORD:-localdev123}' psql -U ${POSTGRES_USER:-prs_user} -d ${POSTGRES_DB:-prs_local} -t -c \"SELECT remaining_gfq FROM items WHERE id = 7;\"" | tr -d ' ')
    local item_28_gfq=$(docker exec prs-local-postgres-timescale bash -c "PGPASSWORD='${POSTGRES_PASSWORD:-localdev123}' psql -U ${POSTGRES_USER:-prs_user} -d ${POSTGRES_DB:-prs_local} -t -c \"SELECT remaining_gfq FROM items WHERE id = 28;\"" | tr -d ' ')
    ...
    local req_status=$(docker exec prs-local-postgres-timescale bash -c "PGPASSWORD='${POSTGRES_PASSWORD:-localdev123}' psql -U ${POSTGRES_USER:-prs_user} -d ${POSTGRES_DB:-prs_local} -t -c \"SELECT status, force_closed_at, force_closed_by FROM requisitions WHERE id = $TEST_REQUISITION_ID;\"")
    ...
    local cancelled_cs=$(docker exec prs-local-postgres-timescale bash -c "PGPASSWORD='${POSTGRES_PASSWORD:-localdev123}' psql -U ${POSTGRES_USER:-prs_user} -d ${POSTGRES_DB:-prs_local} -t -c \"SELECT id, status, cancelled_at FROM canvass_requisitions WHERE requisition_id = $TEST_REQUISITION_ID AND status = 'cs_cancelled';\"")
    ...
    local cancelled_rr=$(docker exec prs-local-postgres-timescale bash -c "PGPASSWORD='${POSTGRES_PASSWORD:-localdev123}' psql -U ${POSTGRES_USER:-prs_user} -d ${POSTGRES_DB:-prs_local} -t -c \"SELECT id, status, cancelled_at FROM delivery_receipts WHERE requisition_id = $TEST_REQUISITION_ID AND status = 'rr_cancelled';\"")
    ...
    local cancelled_ir=$(docker exec prs-local-postgres-timescale bash -c "PGPASSWORD='${POSTGRES_PASSWORD:-localdev123}' psql -U ${POSTGRES_USER:-prs_user} -d ${POSTGRES_DB:-prs_local} -t -c \"SELECT id, status, cancelled_at FROM invoice_reports WHERE requisition_id = $TEST_REQUISITION_ID AND status = 'ir_cancelled';\"")
    ...
    local cancelled_pr=$(docker exec prs-local-postgres-timescale bash -c "PGPASSWORD='${POSTGRES_PASSWORD:-localdev123}' psql -U ${POSTGRES_USER:-prs_user} -d ${POSTGRES_DB:-prs_local} -t -c \"SELECT id, status, cancelled_at FROM rs_payment_requests WHERE requisition_id = $TEST_REQUISITION_ID AND status = 'pr_cancelled';\"")
