...
get_requisition_count() {
    docker exec prs-local-postgres-timescale bash -c "PGPASSWORD='$DB_PASSWORD' psql -U $DB_USER -d $DB_NAME -t -c \"SELECT COUNT(*) FROM requisitions WHERE rs_number LIKE 'TEST-FC-SCENARIO%';\"" | tr -d ' '
}
